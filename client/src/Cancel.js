...
function Cancel() {
  ...
  const trueSubmit = async (e) => {
    ...
    await axios.get(config.urls.webAPI + '/cancel', {params: {id, recipientEmail}}).catch((e) => {throw new Error((e.response.data != null)? e.response.data:'INTERNAL_SERVER_ERROR')});
    return true;
  }
  // Submission control
  const submit = async (e) => {
    ...
    // complete
    setCurrentSuccess(dialogues.success.CANCEL);
    setSubmitting(false);
    return true;
  }
  ...
  useEffect(() => {
    ...
    const modal = new Modal(document.getElementById('cancel'));
    modal.show();
  ...
  return (
    <div className='flex flex-col min-h-screen bg-light_bg text-light_text text-lg'>
      <div id='cancel' aria-hidden='true' className='hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full'>
              ...
              { (currentSuccess !== '')? (<div/>) : (
                  <div>
                    <p className='text-base leading-relaxed text-light_text_l mb-4'>
                      This action cannot be undone. Please verify the email of the recipient of the letter to continue.
                    </p>
